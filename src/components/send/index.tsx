...
import { ModalAlert, ModalPassword } from "../modal/index";
import {
  ...
  useEffect(() => {
    async function getBalance() {
      if (props.asset?.assetId) {
        ...
        setBalance(
          toBiggestCoinUnit(await getLunesBalance(getAddressFromStorage()), 8)
        );
      }
    }
      ...
      <S.SendContainer>
        ...
        <S.TransferDetails>
          <S.Row>
            ...
            <S.Label>{`${balance} ${props.asset?.name || "Lunes"}`}</S.Label>
          </S.Row>

          <S.Row>
            <S.Label>Taxa da rede</S.Label>
            <S.Label>{`${toBiggestCoinUnit(fee, 8)} Lunes`}</S.Label>
          </S.Row>
